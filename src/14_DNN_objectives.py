...
def run(
    ...
    if X_initial_states.shape[0] != X_attacks.shape[0]:
        raise Exception(
            f"Number of initial state ({X_initial_states.shape[0]}) is different from the number of attacks ({X_attacks.shape[0]})"
        )
    ...
    constraints = constraints_evaluator.evaluate(
        scaler.inverse_transform(X_misclassified)
    )
    ...
    X_missclassified_constraints = X_misclassified[
        (1 - constraints_violated).astype(bool)
    ]
    misclasiffication_constraints_rate = (
        X_missclassified_constraints.shape[0] / X_attacks.shape[0]
    )
